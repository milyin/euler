...
fn triangle_numbers() ->TriangleNumbers {
    TriangleNumbers {prev: 0, pos: 0}
}

fn divisors(sieve: &mut Sieve, mut n:i64) -> i64 {
    let mut d = 1 as i64;
    if n > 1 {
        for p in primes(sieve) {
            if p * p > n {
                d *= 2;
                break;
            }
...
fn test(sieve: &mut Sieve, n: i64) -> i64 {
    triangle_numbers()
        .find(|v| divisors(sieve, *v) > n )
        .expect("triangle numbers infinite")
}
...
fn main() -> Result<(), Box<std::error::Error>> {
    ...
    for _ in 0..input_t {
        ...
        writeln!(io::stdout(), "{}", test(&mut sieve, n))?;
    }
    Ok(())
}
